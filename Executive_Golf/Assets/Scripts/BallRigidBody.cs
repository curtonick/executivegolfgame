...
public class BallRigidBody : MonoBehaviour {
	...
	void Start () {
		...
		//playerBody = GameObject.Find ("Player1Body");
		//playerCam = GameObject.Find ("Player1Cam").GetComponent<Camera> ();
		playerSpeed = 1.6f;
	...
	//Deal with player movement while ball is in air
	void Player_Movement () {
		if (Input.GetKey (KeyCode.W)) {
			playerBody.transform.Translate (Vector3.forward * playerSpeed);
		}
		if (Input.GetKey (KeyCode.A)) {
			playerBody.transform.Translate (Vector3.left * playerSpeed *0.8f);
		}
		if (Input.GetKey (KeyCode.S)) {
			playerBody.transform.Translate (Vector3.back * playerSpeed *0.8f);
		}
		if (Input.GetKey (KeyCode.D)) {
			playerBody.transform.Translate (Vector3.right * playerSpeed *0.5f);
		}
	}
	...
	void Update () {
		...
		if (launched) {
			if (Time.time > timeOfShot + (1.0f + Mathf.Abs(clubSelection - full.Length)/8.0f)  * power/104.0f) {
				...
				//follow.rect = new Rect (new Vector2 (0, 0), new Vector2 (0.2f, 0.2f));
